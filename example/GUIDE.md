...
本地运行类型、Lint 和受影响用例；下列完整 Jest、库打包、example 与原生构建命令由 CI 执行。真机测试按任务授权运行对应平台。
...
本仓不使用全局 `logFilters`；`yarn check:runtime-peers` 只接受 root、example、website 三条精确的 RNRC 5 / RNGH 3 例外。harness 不继承本仓 `check:runtime-peers` 的 workspace 精确 allowlist。依赖安装见[接入指南](../website/docs/getting-started.md)，临时验证宿主见[开发资料](../docs/DEVELOPMENT.md#临时验证宿主)。
