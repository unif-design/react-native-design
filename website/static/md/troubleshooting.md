...
本库不发 mock 入口,Jest 里要接的是底层 peer。完整配方见[在宿主工程里测试](testing.md);下面是照着报错反查。
...
**解法。** 把 `@unif/react-native-design` 和它的 RN 生态 peer 一起加进白名单,见[测试 → 最小可用配方](testing.md#最小可用配方)。只加 `@unif/react-native-design` 不够 —— 那几个 peer 同样发 ESM / TS 源码。
...
**解法。** 在 setup 文件里把 RNGH 的 `Pressable` 覆盖成 RN 的(保留 `...actual`,`GestureHandlerRootView` 仍要是真的),见[测试 → 最小可用配方](testing.md#最小可用配方)。
...
**解法。** 别映射 reanimated,只映射 `react-native-worklets`(推荐);或保留映射并自己补上 `useReducedMotion`。两种写法见[测试 → 最小可用配方](testing.md#最小可用配方)。
