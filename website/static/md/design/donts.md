...
硬性禁忌。违反这些会破坏品牌一致性。每条都对应一条[设计原则](principles.md)或 token 规则,载入即生效(load-bearing)。
...
> 详细规则见[设计原则](principles.md)、[颜色 → 取色优先级链](tokens/colors.md#取色优先级链)、[动效](tokens/motion.md);单文件全量 Don'ts 见[完整规范](../UNIF-DESIGN.md)。
