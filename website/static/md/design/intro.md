...
> 找**怎么装、怎么用代码**?去[快速开始](../getting-started.md)。找**逐组件 API**?去[组件概览](../components/overview.md)。要一份单文件全量参考?去[完整规范](../UNIF-DESIGN.md)。
...
完整的 5 条不可违背规则见[设计原则](principles.md)。
...
| [语调与文案](voice.md) | 大小写、人称、状态文案速查、建议 chip 语气、标点 |
| [颜色 token](tokens/colors.md) | role-based 角色 token、亮 / 暗各一套、取色优先级链 |
| [字体 token](tokens/typography.md) | `type` 字号阶梯 + `fw` 字重 + 系统字体栈 |
| [间距 · 圆角 · 阴影](tokens/spacing-radii-shadows.md) | `space` 4px 网格、`radius`(含气泡非对称)、themed `shadow` |
| [动效 token](tokens/motion.md) | `motion` 时长、reanimated 用法 |
| [图标](../components/icons.md) | 24×24 描边 SVG 规则 |
| [全局 Don'ts](donts.md) | 硬性禁忌清单 |
...
- [设计原则](principles.md) —— 5 条不可违背的规则,新增组件 / 改样式前必读
- [颜色 token](tokens/colors.md) —— role-based 角色 token 与取色优先级链
- [组件概览](../components/overview.md) —— 40+ 组件索引,按场景分组
- [快速开始](../getting-started.md) —— 装包、挂 ThemeProvider、5 分钟跑通
