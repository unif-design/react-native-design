...
Unif Design 的颜色是 **role-based(角色化)token**:role 名固定(`primary` / `surface` / `foreground`…),亮 / 暗主题切换时,改的是 role 指向哪个 hex,组件代码不动。具体值见 `src/theme/colors.ts`。
...
**品牌渐变**(数组,不暴露为 role):`avatarGradient = ['#F49443', '#EB6E00']`(135°,Me hero 头像;亮 / 暗共用)。渐变属于明确的组件能力；使用时保留其主题与平台边界。
...
<Swatches
  items={[
    ...
    { name: 'info', hex: '#3775F6', desc: '信息语义色(暗:#5A91FF)' },
  ]}
/>
...
| `info`             | `#3775F6` | `#5A91FF` | 信息提示及支持 info 变体的组件 |
| `infoContainer`    | `#F0F5FF` | `#0F1A33` | info badge 底                  |
| `onInfo`           | `#FFFFFF` | `#FFFFFF` | info 上的内容                  |

`info` 表达信息类语义，使用方式由公开组件的变体决定；它不限定某一种业务身份。参见[颜色使用原则](../principles.md#橙色克制)。
  ...
  items={[
    {
      name: 'background',
      hex: '#F5F5F5',
      desc: '整页底(暗:#0A0A0A,OLED 友好)',
    },
    {
      name: 'surface',
      hex: '#FFFFFF',
      desc: 'Card / Sheet / 气泡(暗:#1C1C1E)',
    },
    {
      name: 'surfaceContainer',
      hex: '#F5F5F5',
      desc: '卡内输入框 / 次级(暗:#2C2C2E)',
    },
    {
      name: 'surfaceContainerHigh',
      hex: '#F0F0F0',
      desc: '次按钮 / 按下态(暗:#3A3A3C)',
    },
    {
      name: 'surfaceContainerHighest',
      hex: '#E0E0E0',
      desc: 'disabled / pill(暗:#48484A)',
    },
  ]}
/>
...
更多使用方式见[快速开始](../../getting-started.md)，旧变量调整见[版本迁移](../../migration.md)。
