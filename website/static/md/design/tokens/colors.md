...
颜色跟随系统亮暗切换,**必须**在组件内用 `useColors()` / `useTheme().colors` 运行期拿。顶层 `import { colors }` 导出已删除(见[迁移 → 令牌迁移](../../migration.md#令牌迁移)）。下方所有色块的 `hex` 仅供查阅;代码里写 role 名,不写 hex。
:::
...
2. **静态 palettes** —— 渐变序列(`avatarGradient`、`warmOrangePalette`),塞不进单值 role 的"色组 / 渐变",见[间距·圆角·阴影](spacing-radii-shadows.md)与下文。
...
:::warning 蓝色规则
`info`(`#3775F6`)**专属于用户头像**。除 Tag 的 `info` variant 历史沿用外,不要在按钮、提示、链接等任何其它地方铺蓝色 —— 这会破坏 user / AI 的视觉区分。详见[设计原则 → 橙色克制](../principles.md#橙色克制)。
:::

## 表面 / Surface(5 层) {#表面}

5 层明度阶梯。**暗色不靠 shadow 表达深度,靠这 5 层 surface 的明度差**(见[阴影](spacing-radii-shadows.md#阴影))。
...
- **layered surface 替代 shadow** —— 暗色下 shadow token 的 `shadowOpacity` / `elevation` 多数置 0,深度靠 surface 5 层明度差(见[阴影](spacing-radii-shadows.md#阴影))。
- **气泡"内角方"签名完整保留**(见[设计原则 → 气泡内角方](../principles.md#气泡内角方))。

> 完整 role → hex 对照、迁移 grep 表、为何不引入 reference layer,见[完整规范 → Color system](../../UNIF-DESIGN.md)。
