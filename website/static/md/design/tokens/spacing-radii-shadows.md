...
直角指向头像,是设计系统最辨识度的视觉符号,**不要破坏**(见[设计原则 → 气泡内角方](../principles.md#气泡内角方))。
...
中性卡片阴影 + 品牌橙光晕两类。**无内阴影、无霓虹光晕、无双重边框。** `shadow` 是 **themed token** —— 走 `useShadow()` / `useThemedStyles` 第二参 `s` 拿,**暗色下绝大多数 `shadowOpacity` / `elevation` 自动置 0**(深度靠 surface 5 层明度差表达,见[颜色 → 表面](colors.md#表面))。
...
> 完整 token 速查与暗色哲学,见[完整规范](../../UNIF-DESIGN.md)的 Spacing / Radii / Shadows 三节。
