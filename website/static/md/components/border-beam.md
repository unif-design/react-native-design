...
description: '沿内容边缘循环移动的装饰性流光，可用于图片处理等短时忙碌状态；native 使用 Reanimated，Web 使用 CSS keyframes。'
---

# BorderBeam 边框流光

`BorderBeam` 在既有内容外围叠加一段沿边缘循环移动的流光。它只负责视觉反馈，不改变内容布局，也不承载加载、错误、焦点或选中语义。
...
| `duration`     | `number?`               | `6000`      | 绕边一周时长，单位毫秒；收敛到 `300..60000`      |
| `lineWidth`    | `number?`               | `1`         | 描边宽度；收敛到 `0.5..8`                        |
