...
下方在 Web 平台解析到 `ToastHost.web.tsx`：使用纯 React state、CSS transition 与 timer 驱动 fade + slide；native 才解析到 `ToastHost.tsx` 并使用 Reanimated 4。两端共享同一个 Toast Store 与 delivery identity 契约。点按钮触发。
