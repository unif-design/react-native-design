...
- action slide：必须同时传入 `onPressItem` 与 `getAccessibilityLabel`，每张 slide 才是 RN core `<Pressable accessibilityRole="button">`。这是 Carousel 内与第三方 `GestureDetector` 兼容的窄例外。
- action 名称固定拼为 `${getAccessibilityLabel(item, index)}，第 ${index + 1} 项，共 ${data.length} 项`，业务名称不能缺失。
