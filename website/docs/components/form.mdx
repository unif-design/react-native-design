...
- 真正的 a11y 语义来自传入 `FormRow` 的**控件 children**(如 [Input](./input) 的 `<TextInput>`、[Switch](./switch)),由控件自身承载 role / label / state；Switch 的 `accessibilityLabel` 是必填，必须与行标题一致或提供更明确的名称。
