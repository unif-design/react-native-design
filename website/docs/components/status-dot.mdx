...
`tone='flat'`（默认）—— pending/active 底色透明，用于 [Task](/docs/chat/task)。
`tone='soft'` —— pending = `surface`、active = `primaryContainer`，用于 [ChainOfThought](/docs/chat/chain-of-thought)（推理链气氛更柔）。
...
- [Task](/docs/chat/task) `TaskRow` —— `<StatusDot status={mapped} size={16} />`（todo→pending、running→active、done→done）
- [ChainOfThought](/docs/chat/chain-of-thought) `Step` —— `<StatusDot status={step.status} size={18} tone="soft" />`
