...
sidebar_position: 6
title: Confirm 确认对话框
description: "命令式 await confirm() → Promise<boolean> —— 高风险操作二次确认，BottomSheetModal 单档 + ConfirmHost Portal，destructive 红确认按钮，同一时间仅 1 个。"
---
...
- 对话框容器走 `@gorhom/bottom-sheet` 的 `<BottomSheetModal>`，源码未在其上显式设 `accessibilityRole`/`accessibilityViewIsModal`，沿用该库的默认行为。
...
| 按钮行 | `<Button variant="secondary" block>` 取消 + `<Button variant="primary"\|"danger" block>` 确认,横向均分(走 BottomSheetFooter) |
...
- BottomSheet — Portal 底层（`@gorhom/bottom-sheet` wrap，见 `src/components/ui/BottomSheet/`）
