...
- 取消 / 确认两个按钮用本库 [Button](./button) 渲染。自定义 `cancelLabel` / `confirmLabel` 会先 trim；空白值分别回退“取消”/“确认”，因此不会把空名称传给 Button。
...
接管瞬间前任手里那个未决对话框会立即 `resolve(false)` 并关闭 —— 这是刻意的:Promise 不悬挂、单例槽不被带走,新 Host 一上来就能弹。归还(Modal 关闭)方向同样结算成 `false`:confirm 的上下文随 Modal 一起消失了,当作取消处理。这一点和 Toast 不同 —— toast 归还时会把没播完的那条交回上一层重播,详见 [Toast → 投递语义](/docs/components/toast#投递语义)。
