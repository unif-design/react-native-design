...
| 节点 | testID | 预期布局 |
|---|---|---|
| 左右 outer RNGH Pressable | `stepper-middle-decrement` / `stepper-middle-increment` | side outer = max(44, visual button width) × max(44, visual height)，透明、无 `hitSlop`；visual 朝中央贴边 |
