...
- actionable 是唯一外层可访问操作：RNGH `Pressable` 固定为 `button`。默认名称依次组合非空的 `title`、`desc` 与 extra 文本，使用中文顿隔 `，`；actionable display 的 `accessibilityText` 只有非空白时才贡献。显式 `accessibilityLabel` 可覆盖。
