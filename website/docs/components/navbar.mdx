...
> `left` / `right` 传完整 `NavBarAction` 时渲染为内部 `IconButton`（`variant='ghost'`，icon 必须是生成的 `IconName`，色按 variant tint：brand 用 `c.onPrimary`、其余 `c.foreground`）；展示节点中的 `string` / `number` / `bigint`（包括 `0`）会递归包装为本地 `Text`，React element / Promise / portal 保持原有语义。缺字段、非法 icon 或伪造 React 标记的非类型安全 plain object 会在 effect 中诊断一次并渲染为空，不会伪造 handler 或英文 icon 名。标题用 `t.h3 + 0.5` / `fw.semi` / `letterSpacing: -0.2`，副标题 `t.micro`（11）。
