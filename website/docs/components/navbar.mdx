...
> `left` / `right` 传完整 `NavBarAction` 时渲染为内部 `IconButton`（`variant='ghost'`，icon 必须是生成的 `IconName`，色按 variant tint：brand 用 `c.onPrimary`、其余 `c.foreground`）；展示节点直接原样渲染（包括 `0`）。缺字段、非法 icon 或伪造 React 标记的非类型安全 plain object 会在 effect 中诊断一次并渲染为空，不会伪造 handler 或英文 icon 名。标题用 `t.h3 + 0.5` / `fw.semi` / `letterSpacing: -0.2`，副标题 `t.micro`（11）。
...
- a11y props：`left` / `right` 槽的 `NavBarAction.accessibilityLabel` **必填且非空**；action 同时必须有函数 `onPress`。若直接传 `ReactNode`，则 a11y 由该节点自行负责。
