...
| `status` | `VersionStatus?` | `{ label: '正常', color: c.success }` | 可见状态文案与状态点颜色；caller 提供状态但省略 `color` 时使用 `c.foregroundMuted` |
...
VersionPill 是**纯展示**徽章，外层本地 `<View accessible>` 合并名称，例如“版本 2.0.0，build 12，测试中”；没有 build 时对应片段整体省略。分隔使用全角逗号“，”。状态点、可见状态文字、版本和 build 子节点均只在本地 RN `View` / `Text` 上使用共享隐藏属性，避免重复焦点。组件未设置操作 role，也没有受控状态。
