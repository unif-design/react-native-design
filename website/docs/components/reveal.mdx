...
Web 不渲染额外 `<div>` 或 wrapper；`style`、`testID` 和 children 全部落在同一个 RN View。动画使用 `[callerStyle, animatedStyle]`，从 `opacity: 0` 过渡到 caller 最终合法 opacity（`0..1`），而不是固定到 `1`。两个 RAF 分别跟踪并在 cleanup 取消，generation guard 阻止旧回调写入新一轮；RAF API 不可用时失败关闭为立即显示。
