  ...
  return (
    <LiveDemo>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <List>
          <Cell title="拜访提醒" desc="按计划自动推送提醒" extra={<Switch value={notify} onChange={setNotify} accessibilityLabel="拜访提醒" />} />
          <Cell title="同步给团队" desc="拜访完成后通知主管" extra={<Switch value={team} onChange={setTeam} accessibilityLabel="同步给团队" />} />
          <Cell title="语音输入" desc="长按说话输入消息" extra={<Switch value={voice} onChange={setVoice} accessibilityLabel="语音输入" />} />
        </List>
      </div>
    </LiveDemo>
  );
...
| 节点 | testID | 预期布局 |
|---|---|---|
| outer RNGH Pressable | `selection-switch` | 44×44，透明、无 `hitSlop` |
| 本地 visual wrapper | `selection-switch-visual` | 32×20，位于 outer 正中央 |
