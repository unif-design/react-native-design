...
- RN `0.86.3` / React `19.2.3`,新架构(Fabric + TurboModules)默认开启。发布 contract 为 `react-native >=0.86.0`(不封顶,验证基线为 `0.86.3`)、`react >=19.2.3 <20.0.0`;Node engine 为 `^20.19.4 || ^22.13.0 || ^24.3.0 || >= 25.0.0`。运行时四件套固定在 RNGH `>=3.0.0 <4.0.0`、Reanimated `>=4.5.2 <4.7.0`、Worklets `>=0.11.0 <0.13.0`、RNRC `>=5.0.0 <6.0.0`。
